...
plotClusterCustom <- function(cluster, regions, titleName, coverageInfo, groupInfo, titleUse="fwer", txdb=NULL, p.ideogram=NULL, maxExtend=300L, colsubset=NULL, forceLarge=FALSE) {
	...
	if(!is.logical(p.transcripts)) {
		result <- tracks(p.ideogram, "Coverage" = p.coverage, "Mean coverage" = p.meanCov, "Regions" = p.region, "tx_name\n(gene_id)" = p.transcripts, heights = c(2, 4, 4, 1.5, 3), xlim=wh, title=title) + ylab("")
	} else {
		result <- tracks(p.ideogram, "Coverage" = p.coverage, "Mean coverage" = p.meanCov, "Regions" = p.region, heights = c(2, 5, 5, 2), xlim=wh, title=title) + ylab("")
	}
	return(result)	
}

## Plotting function
regionClusterPlot <- function(i, tUse="fwer", sunset = TRUE) {
    ...
    if(sunset) p <- p + theme_tracks_sunset()
	print(p)
	rm(p.ideo, covInfo)
...
fig2 <- regionClusterPlot(2, sunset = FALSE) + theme_classic()
fig3 <- regionClusterPlot(7, sunset = FALSE) + theme_classic()
pdf('figure2.pdf', w = 10, h = 7)
fig2 + theme(text = element_text(size = 16), axis.text.x = element_text(size = 10), legend.key = element_blank())
dev.off()
pdf('figure3.pdf', w = 10, h = 7)
fig3 + theme(text = element_text(size = 16), axis.text.x = element_text(size = 10), legend.key = element_blank())
dev.off()
```
